...
def migrate_scene_settings(scene: bpy.types.Scene) -> None:
    ...
    if current < 604:
        settings.cache_status = "Updated to 0.6.4 conservative Compound Convex proxies; clear all caches and rebake"
    ...
    if current < 700:
        try:
            stored_bonds = json.loads(str(getattr(settings, "bond_data", "") or "[]"))
            settings.bond_count = len(stored_bonds) if isinstance(stored_bonds, list) else 0
        except Exception:
            settings.bond_data = ""
            settings.bond_count = 0
        settings.bond_enabled = True
        settings.cache_status = "Updated to 0.7.0 breakable bonds; regenerate bonds and rebake"
        migration_detail += " Applied 0.7.0 breakable Fixed-bond authoring and impact-load fracture defaults."

    if current < 701:
        settings.bond_stability_mode = "RIGID"
        settings.cache_status = "Updated to 0.7.1 rigid bond islands; clear cache and rebake"
        migration_detail += (
            " Applied 0.7.1 rigid bond-island stabilization, deterministic constraint backbones "
            "and moving bond anchors."
        )

    if current < 702:
        settings.cache_status = "Updated to 0.7.2 bond collision filtering; clear cache and rebake"
        migration_detail += (
            " Applied 0.7.2 intact bond-island self-collision filtering and topology-aware "
            "collision restoration after fracture."
        )

    if current < 703:
        settings.cache_status = "Updated to 0.7.3 native bond islands; clear cache and rebake"
        migration_detail += (
            " Applied 0.7.3 native constraint-only rigid bond stabilization and "
            "whole-island sleeping without post-step transform projection."
        )

    if current < 704:
        settings.cache_status = "Updated to 0.7.4 rigid compound bond islands; clear cache and rebake"
        migration_detail += (
            " Applied 0.7.4 one-body rigid compound bond islands with deterministic "
            "actor splitting after bond-graph disconnection."
        )

    if current < 705:
        settings.cache_status = "Updated to 0.7.5 safe rigid-cluster hull proxies; clear cache and rebake"
        migration_detail += (
            " Applied 0.7.5 interior sphere-cloud proxies for single-hull fallbacks "
            "inside rigid bond compounds, preventing start-frame ground depenetration jumps."
        )

    if current < 706:
        settings.cache_status = "Updated to 0.7.6 authored rigid-ground rest; clear cache and rebake"
        migration_detail += (
...
def load_scene_cache(scene: bpy.types.Scene) -> Optional[Dict]:
    ...
    expected = {"addon_version": ADDON_VERSION, "culverin_version": BUNDLED_CULVERIN_VERSION}
    mismatches = {
