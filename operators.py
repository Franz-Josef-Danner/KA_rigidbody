...
from .core.bonds import clear_bonds, generate_proximity_bonds, load_bonds, save_bonds
...
def _payload_summary(payload):
    ...
    for body in bodies:
        ...
        compound_bodies += int(shape in {"COMPOUND", "COMPOUND_CONVEX"})
...
def _compound_runtime_guard_fallbacks(payload: Dict, result: Dict):
    ...
    compound_names = {
        str(body.get("name"))
        for body in payload.get("bodies", [])
        if str(body.get("collision_shape")) in {"COMPOUND", "COMPOUND_CONVEX"}
    }
    candidates = []
    fallback_names = set()
...
def _compound_initial_overlap_fallbacks(payload: Dict, result: Dict):
    """Detect an implausibly dense first-frame compound contact burst.

    Fracture pieces may legitimately begin in light contact, but hundreds of new
    compound contacts in the first rendered frame indicate collider overfill.
    In that case all compound bodies are rebuilt as their already-prepared single
    convex hulls before the cache is accepted.
    """
    ...
    totals = dict(result.get("diagnostic_totals") or {})
    contacts = dict(totals.get("first_simulated_frame_contacts") or {})
    state = dict(totals.get("first_simulated_frame_state") or {})
    compound_count = len(compound_names)
    added = int(contacts.get("contact_added", 0) or 0)
    events = int(contacts.get("contact_events", 0) or 0)
    added_limit = max(24, compound_count * 2)
    event_limit = max(128, compound_count * 20)
    triggered = bool(compound_count and (added > added_limit or events > event_limit))
    report = {
        "compound_count": compound_count,
        "contact_added": added,
        "contact_events": events,
        "contact_added_limit": added_limit,
        "contact_event_limit": event_limit,
        "max_linear_speed": float(state.get("max_linear_speed", 0.0) or 0.0),
        "max_angular_speed": float(state.get("max_angular_speed", 0.0) or 0.0),
        "triggered": triggered,
    }
    return (compound_names if triggered else []), report


def _force_single_hull_fallback(
    payload: Dict,
    body_names,
    *,
    ...
    changed = 0
    for body in payload.get("bodies", []):
        if str(body.get("name")) not in names or str(body.get("collision_shape")) not in {"COMPOUND", "COMPOUND_CONVEX"}:
            continue
    ...
    simulation_scene = payload.get("simulation_scene")
    if isinstance(simulation_scene, dict):
        scene_changed = apply_single_hull_fallback(
            simulation_scene,
            payload.get("bodies", []),
            names,
            reason=reason,
        )
        # Both views must agree. A mismatch is safer as a failed fallback than
        # silently rebaking the original compound through solver_payload().
        if scene_changed != changed:
            raise RuntimeError(
                f"SimulationScene fallback mismatch: legacy={changed}, scene={scene_changed}"
            )

    profile = payload.get("build_profile")
    if isinstance(profile, dict):
        profile["compound_runtime_fallbacks"] = changed
        profile["compound_runtime_fallback_reason"] = str(reason)
    return changed
...
class KA_RIGID_OT_generate_bonds(Operator):
    ...
    bl_description = "Connect nearby enabled meshes with breakable Fixed constraints"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        if self.selected_only:
            ...
            objects = [
                obj for obj in enabled_body_objects(scene)
                if obj.type == "MESH" and not is_ka_ground(obj)
            ]
        if len(objects) < 2:
            self.report({"WARNING"}, "At least two enabled mesh bodies are required")
            return {"CANCELLED"}
        try:
            bonds, report = generate_proximity_bonds(
                objects,
                context.evaluated_depsgraph_get(),
                maximum_distance=float(world.bond_connection_distance),
                break_force=float(world.bond_break_force),
                break_torque=float(world.bond_break_torque),
                damage_accumulation=float(world.bond_damage_accumulation),
            )
        except Exception as exc:
            log_exception(scene, "BONDS", "GENERATION_FAILED", exc)
        ...
        if self.selected_only:
            ...
            retained = [
                bond for bond in load_bonds(scene)
                if not (
                    str(bond.get("body_a", "")) in selected_ids
                    and str(bond.get("body_b", "")) in selected_ids
                )
            ]
            bonds = retained + bonds
        save_bonds(scene, bonds)
        world.bond_enabled = True
        if world.backend != "JOLT":
            status = get_backend("JOLT").status(addon_preferences(context))
            if status.available:
                world.backend = "JOLT"
            else:
                self.report({"WARNING"}, "Bonds were generated, but Jolt is currently unavailable")
        world.cache_status = "Bond graph changed; bake required"
        log_event(
            scene,
            "BONDS",
            "GENERATED",
            selected_only=bool(self.selected_only),
            break_force=float(world.bond_break_force),
            break_torque=float(world.bond_break_torque),
            damage_accumulation=float(world.bond_damage_accumulation),
            **report,
...
class KA_RIGID_OT_clear_bonds(Operator):
    bl_idname = "ka_rigid.clear_bonds"
    bl_label = "Clear Bonds"
    bl_description = "Remove the persisted breakable bond graph"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        scene = context.scene
        count = clear_bonds(scene)
        scene.ka_rigid_world.cache_status = "Bond graph cleared; bake required"
        log_event(scene, "BONDS", "CLEARED", count=count)
        self.report({"INFO"}, f"Cleared {count} bonds")
        return {"FINISHED"}
...
class KA_RIGID_OT_bake(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            if bool(world.bond_enabled) and int(world.bond_count) > 0 and world.backend != "JOLT":
                status = get_backend("JOLT").status(addon_preferences(context))
                if not status.available:
                    raise BackendError(f"Breakable bonds require Jolt: {status.detail}")
                world.backend = "JOLT"
                world.cache_status = "Jolt selected for breakable bonds; rebake required"
                log_event(
                    scene,
                    "BACKEND",
                    "AUTO_SELECTED",
                    source="BONDS",
                    previous_backend=requested_backend,
                    effective_backend="JOLT",
        ...
        try:
            ...
            payload["native_jolt_bridge_path"] = str(getattr(preferences, "jolt_bridge_path", "") or "")
            payload_seconds = time.perf_counter() - payload_started
            compound_present = any(
                str(body.get("collision_shape")) in {"COMPOUND", "COMPOUND_CONVEX"}
                for body in payload.get("bodies", [])
            )
            compound_guard_active = bool(world.compound_runtime_guard and compound_present)
            initial_overlap_guard_active = bool(compound_present)
            detailed_contacts = bool(world.detailed_contact_diagnostics)
            detailed_payload = bool(world.detailed_payload_diagnostics)
            collect_contacts = bool(detailed_contacts or compound_guard_active or initial_overlap_guard_active)
            payload["diagnostics"] = {
                "enabled": bool(world.log_output),
                "path": log_file_path(scene),
                "contacts": collect_contacts,
                "log_contacts": detailed_contacts,
                "force_contacts": bool(compound_guard_active or initial_overlap_guard_active),
                "contact_reason": (
                    "detailed_contact_diagnostics_and_compound_guards"
                    if detailed_contacts and (compound_guard_active or initial_overlap_guard_active)
                    else "compound_initial_overlap_and_side_stick_guard"
                    if compound_guard_active
                    else "compound_initial_overlap_guard"
                    if initial_overlap_guard_active
            ...
            runtime_guard_report = {
                "enabled": bool(compound_guard_active or initial_overlap_guard_active),
                "initial_overlap_enabled": bool(initial_overlap_guard_active),
                "side_stick_enabled": bool(compound_guard_active),
                "triggered": False,
                "resolved": True,
                "fallback_bodies": [],
                "initial_overlap": {},
            ...
            initial_fallback_names, initial_overlap_report = _compound_initial_overlap_fallbacks(payload, result)
            runtime_guard_report["initial_overlap"] = initial_overlap_report
            if initial_fallback_names:
                changed = _force_single_hull_fallback(
                    payload, initial_fallback_names, reason="initial_overlap_guard"
                )
                runtime_guard_report.update({
                    "triggered": bool(changed),
                    "fallback_bodies": initial_fallback_names,
                })
                log_event(
                    scene,
                    "QUALITY",
                    "COMPOUND_INITIAL_OVERLAP_FALLBACK",
                    level="WARNING",
                    fallback_bodies=initial_fallback_names,
                    **initial_overlap_report,
                )
                world.cache_status = f"Overlap guard rebaking {changed} bodies as Single Hull"
                backend_started = time.perf_counter()
                result = backend.bake(payload, progress=progress)
                backend_seconds += time.perf_counter() - backend_started
            ...
            if compound_guard_active:
                ...
                if fallback_names:
                    ...
                    combined = sorted(set(runtime_guard_report.get("fallback_bodies", [])) | set(fallback_names))
                    runtime_guard_report.update({
                        "triggered": bool(runtime_guard_report.get("triggered") or changed),
                        "fallback_bodies": combined,
    ...
    def execute(self, context):
        ...
        try:
            ...
            payload["diagnostics"] = {
                "enabled": bool(world.log_output),
                "path": log_file_path(scene),
                "contacts": bool(detailed_contacts or compound_guard_active or initial_overlap_guard_active),
                "log_contacts": detailed_contacts,
                "force_contacts": bool(compound_guard_active or initial_overlap_guard_active),
                "contact_reason": (
                    "detailed_contact_diagnostics_and_compound_guards"
