...
class KA_RIGID_AddonPreferences(AddonPreferences):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Jolt is bundled for Blender CPython 3.13.", icon="INFO")
        box.label(text="Supported: Windows x64 and Linux x64.")
        box.label(text="The custom path is reserved for the future PhysX backend.")
...
class KA_RIGID_WorldSettings(PropertyGroup):
    ...
    compound_inset: FloatProperty(
        ...
        description="Shrink each convex piece slightly to prevent contacts between fixed sibling parts",
    )
