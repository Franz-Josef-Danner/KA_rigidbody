...
class JoltBackend(PhysicsBackend):
    ...
    def bake(self, scene_payload: Dict, progress: ProgressCallback = None) -> Dict:
            ...
            for _substep in range(1, frame_substeps + 1):
                ...
                if runtime_bonds:
                    ...
                    if bond_stability_mode == "RIGID":
                        projection = self._project_intact_bond_islands(world, runtime_bonds)
                        totals["bond_projection_passes"] += 1
                        totals["bond_projection_bodies"] += int(projection.get("projected_bodies", 0))
                        totals["bond_projection_max_correction"] = max(
                            float(totals.get("bond_projection_max_correction", 0.0)),
                            float(projection.get("maximum_correction", 0.0)),
                        )
        ...
        complete_log = {
            ...
            "limitations": [
                ...
                *(
                    ["Culverin exposes at most 256 Fixed constraints; the add-on therefore uses a deterministic spanning backbone and rigidly projects the complete intact bond graph."]
                    if runtime_bonds and int(bond_constraint_stats.get("graph_bonds", 0)) > int(bond_constraint_stats.get("created_constraints", 0))
                    else []
                ),
                *(
                    []
    ...
    def _create_breakable_bonds(
        ...
        if omitted:
            warnings.append(
                ...
                "The remaining graph edges are enforced by rigid island stabilization and still participate in fracture topology."
            )
        if int(selection_stats.get("required_backbone_edges", 0)) > created:
            warnings.append(
                "The native constraint budget is smaller than the bond graph's spanning forest. "
                "Some rigidly projected regions cannot transfer impact impulses through a complete native constraint path."
            )
        stats: Dict[str, Any] = {
    ...
    def _body_pose_jolt(
        ...
        result = (tuple(map(float, position[:3])), _quat_normalize_xyzw(rotation))
    ...
    @classmethod
    def _project_intact_bond_islands(
        cls,
        world,
        bonds: Sequence[_RuntimeBond],
    ) -> Dict[str, Any]:
        """Remove visible stretch from every intact connected bond island.

        Native Fixed constraints still transfer impulses and establish solver
        islands. This deterministic post-step projection fits one rigid transform
        to the current member poses, so no arbitrary fragment becomes a motion
        driver and linear momentum from impacts is not discarded.
        """
        intact = [bond for bond in bonds if not bond.broken]
        if not intact:
            return {"components": 0, "projected_bodies": 0, "maximum_correction": 0.0}
        adjacency: Dict[str, List[_RuntimeBody]] = {}
        bodies: Dict[str, _RuntimeBody] = {}
        degree: Dict[str, int] = {}
        for bond in intact:
            a = bond.body_a
            b = bond.body_b
            bodies[a.stable_id] = a
            bodies[b.stable_id] = b
            adjacency.setdefault(a.stable_id, []).append(b)
            adjacency.setdefault(b.stable_id, []).append(a)
            degree[a.stable_id] = degree.get(a.stable_id, 0) + 1
            degree[b.stable_id] = degree.get(b.stable_id, 0) + 1

        visited: set[str] = set()
        components = 0
        projected_bodies = 0
        maximum_correction = 0.0
        for start_id in sorted(bodies):
            if start_id in visited:
                continue
            stack = [bodies[start_id]]
            component: List[_RuntimeBody] = []
            visited.add(start_id)
            while stack:
                current = stack.pop()
                component.append(current)
                for neighbour in adjacency.get(current.stable_id, ()):
                    if neighbour.stable_id not in visited:
                        visited.add(neighbour.stable_id)
                        stack.append(neighbour)
            dynamic = [body for body in component if body.body_type == "DYNAMIC"]
            if len(component) <= 1 or not dynamic:
                continue
            components += 1
            statics = [body for body in component if body.body_type == "STATIC"]
            reference = max(
                statics or component,
                key=lambda body: (
                    int(degree.get(body.stable_id, 0)),
                    float(body.mass),
                    body.stable_id,
                ),
            )
            inverse_rest_reference = _quat_conjugate_xyzw(reference.rest_rotation_jolt)
            samples: List[Dict[str, Any]] = []
            any_active = False
            for body in component:
                position = tuple(map(float, (world.get_position(body.handle) or body.rest_position_jolt)[:3]))
                rotation = _quat_normalize_xyzw(world.get_rotation(body.handle) or body.rest_rotation_jolt)
                rest_offset = _quat_rotate_xyzw(
                    inverse_rest_reference,
                    subtract_vec3(body.rest_position_jolt, reference.rest_position_jolt),
                )
                rest_rotation = _quat_normalize_xyzw(
                    _quat_multiply_xyzw(inverse_rest_reference, body.rest_rotation_jolt)
                )
                mass = max(1.0e-8, float(body.mass)) if body.body_type == "DYNAMIC" else 0.0
                ...
                samples.append({
                    ...
                    "mass": mass,
                    "linear": tuple(map(float, linear[:3])),
                    "angular": tuple(map(float, angular[:3])),
                })

            if statics:
                reference_sample = next(sample for sample in samples if sample["body"] is reference)
                cluster_rotation = reference_sample["rotation"]
                cluster_position = reference_sample["position"]
            else:
                total_mass = sum(float(sample["mass"]) for sample in samples)
                if total_mass <= 1.0e-12:
                    total_mass = float(len(samples))
                    for sample in samples:
                        sample["mass"] = 1.0
                candidates: List[Tuple[Tuple[float, float, float, float], float]] = []
                for sample in samples:
                    candidate = _quat_normalize_xyzw(
                        _quat_multiply_xyzw(
                            sample["rotation"],
                            _quat_conjugate_xyzw(sample["rest_rotation"]),
                        )
                    )
                    candidates.append((candidate, float(sample["mass"])))
                reference_quaternion = candidates[0][0]
                qsum = [0.0, 0.0, 0.0, 0.0]
                for candidate, weight in candidates:
                    if sum(candidate[index] * reference_quaternion[index] for index in range(4)) < 0.0:
                        candidate = tuple(-value for value in candidate)
                    for index in range(4):
                        qsum[index] += candidate[index] * weight
                cluster_rotation = _quat_normalize_xyzw(qsum)
                position_sum = [0.0, 0.0, 0.0]
                for sample in samples:
                    fitted_offset = _quat_rotate_xyzw(cluster_rotation, sample["rest_offset"])
                    candidate_position = subtract_vec3(sample["position"], fitted_offset)
                    weight = float(sample["mass"])
                    for index in range(3):
                        position_sum[index] += candidate_position[index] * weight
                cluster_position = tuple(value / total_mass for value in position_sum)

            total_dynamic_mass = sum(float(sample["mass"]) for sample in samples if sample["body"].body_type == "DYNAMIC")
            total_dynamic_mass = max(1.0e-12, total_dynamic_mass)
            linear_sum = [0.0, 0.0, 0.0]
            angular_sum = [0.0, 0.0, 0.0]
            targets: Dict[str, Tuple[Tuple[float, float, float], Tuple[float, float, float, float]]] = {}
            target_com_sum = [0.0, 0.0, 0.0]
            for sample in samples:
                body = sample["body"]
                target_position = add_vec3(
                    cluster_position,
                    _quat_rotate_xyzw(cluster_rotation, sample["rest_offset"]),
                )
                target_rotation = _quat_normalize_xyzw(
                    _quat_multiply_xyzw(cluster_rotation, sample["rest_rotation"])
                )
                targets[body.stable_id] = (target_position, target_rotation)
                if body.body_type != "DYNAMIC":
                    continue
                weight = float(sample["mass"])
                for index in range(3):
                    linear_sum[index] += sample["linear"][index] * weight
                    angular_sum[index] += sample["angular"][index] * weight
                    target_com_sum[index] += target_position[index] * weight
            component_linear = tuple(value / total_dynamic_mass for value in linear_sum)
            component_angular = tuple(value / total_dynamic_mass for value in angular_sum)
            target_com = tuple(value / total_dynamic_mass for value in target_com_sum)

            for sample in samples:
                body = sample["body"]
                if body.body_type != "DYNAMIC":
                    continue
                target_position, target_rotation = targets[body.stable_id]
                maximum_correction = max(
                    maximum_correction,
                    length_vec3(subtract_vec3(target_position, sample["position"])),
                )
                world.set_transform(body.handle, target_position, target_rotation)
                world_offset = subtract_vec3(target_position, target_com)
                target_linear = add_vec3(component_linear, _cross_vec3(component_angular, world_offset))
                world.set_linear_velocity(body.handle, *target_linear)
                world.set_angular_velocity(body.handle, *component_angular)
                if any_active:
                    world.activate(body.handle)
                else:
                    try:
                        world.deactivate(body.handle)
                    except Exception:
                        pass
                projected_bodies += 1
        return {
